...
module.exports = {
  ...
  checkIfDeploymentIsNecessary(objects, funcLastModifiedDate) {
    if (objects && objects.length) {
      ...
      // TODO: ここで .serverless しかみてないのを修正する
      const serverlessDirPath = this.options.package ?
        path.join(this.serverless.config.servicePath, this.options.package) : path.join(this.serverless.config.servicePath, '.serverless')
